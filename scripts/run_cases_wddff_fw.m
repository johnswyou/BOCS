...
%     hs.sdp      = cell(length(lambda_vals));
    ...
    for l=1:length(lambda_vals)
        ...
        % Run BOCS_categorical
        % ====================

        % Run BOCS_categorical with Bayesian model
        inputs_t.estimator = 'bayes';

        bayes.stSA1{l} = BOCS_categorical(inputs_t.model, penalty, inputs_t, 1, 'SA');
        fprintf('Bayes.SA1 - Runtime = %f\n', sum(bayes.stSA1{l}.runTime));

        bayes.stSA2{l} = BOCS_categorical(inputs_t.model, penalty, inputs_t, 2, 'SA');
        fprintf('Bayes.SA2 - Runtime = %f\n', sum(bayes.stSA2{l}.runTime));

        bayes.stSA3{l} = BOCS_categorical(inputs_t.model, penalty, inputs_t, 3, 'SA');
        fprintf('Bayes.SA3 - Runtime = %f\n', sum(bayes.stSA3{l}.runTime));

%         bayes.sdp{l} = BOCS_categorical(inputs_t.model, penalty, inputs_t, 2, 'sdp');
%         fprintf('Bayes.SDP - Runtime = %f\n', sum(bayes.sdp{l}.runTime));

        % Run BOCS_categorical with Horseshoe model
        inputs_t.estimator = 'horseshoe';

        hs.stSA1{l} = BOCS_categorical(inputs_t.model, penalty, inputs_t, 1, 'SA');
        fprintf('HS.SA1 - Runtime = %f\n', sum(hs.stSA1{l}.runTime));

        hs.stSA2{l} = BOCS_categorical(inputs_t.model, penalty, inputs_t, 2, 'SA');
        fprintf('HS.SA2 - Runtime = %f\n', sum(hs.stSA2{l}.runTime));

        hs.stSA3{l} = BOCS_categorical(inputs_t.model, penalty, inputs_t, 3, 'SA');
        fprintf('HS.SA3 - Runtime = %f\n', sum(hs.stSA3{l}.runTime));

%         hs.sdp{l} = BOCS_categorical(inputs_t.model, penalty, inputs_t, 2, 'sdp');
%         fprintf('HS.SDP - Runtime = %f\n', sum(hs.sdp{l}.runTime));

        % Run BOCS_categorical with MLE model
        inputs_t.estimator = 'mle';

        mle.stSA1{l} = BOCS_categorical(inputs_t.model, penalty, inputs_t, 1, 'SA');
        fprintf('MLE.SA1 - Runtime = %f\n', sum(mle.stSA1{l}.runTime));

        mle.stSA2{l} = BOCS_categorical(inputs_t.model, penalty, inputs_t, 2, 'SA');
        fprintf('MLE.SA2 - Runtime = %f\n', sum(mle.stSA2{l}.runTime));

        mle.stSA3{l} = BOCS_categorical(inputs_t.model, penalty, inputs_t, 3, 'SA');
        fprintf('MLE.SA3 - Runtime = %f\n', sum(mle.stSA3{l}.runTime));

%         mle.sdp{l} = BOCS_categorical(inputs_t.model, penalty, inputs_t, 2, 'sdp');
...
%         fprintf('SMAC - Runtime = %f\n', sum(smac{l}.runTime));
        ...
        sa{l}  = simulated_annealing_categorical(objective, inputs_t);
...
%             rnd, sa, bo, ols, smc, smac, bayes, mle, hs, inputs_t);
        iSave1(sprintf(['./results/' test_name '/test%d.mat'], t), ...
            rnd, sa, bo, bayes, mle, hs, inputs_t);
