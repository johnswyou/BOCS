...
def simulated_annealing(objective, inputs):
    ...
    for t in range(n_iter):
        ...
        # Find new sample
        flip_bit = np.random.randint(low = 0, high = 8, size = 1)
        new_x = old_x.copy()
        if flip_bit <= 3:
            new_x[0, flip_bit] = float(np.random.randint(low = 1, high = 7, size = 1))
        elif flip_bit > 3:
            new_x[0, flip_bit] = float(np.random.randint(low = 1, high = 21, size = 1))
        else:
            raise ValueError
